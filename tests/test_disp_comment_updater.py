...
- slot_map vacÃ­o (early return).
- slot 0 sin/con MLC existente.
- slot nuevo, slot existente sin MLC, slot existente con MLC.
- eliminaciÃ³n de MLCs huÃ©rfanos.
- idempotencia.
- escape de caracteres, sanitizaciÃ³n, codificaciÃ³n.
"""
...
# ("MLC_old0") y un comentario de cabecera, slot 1 con asignaciÃ³n sin MLC.
...
# S7res mÃ­nimo con MLC de cabecera + el del slot 0.
...
# S7dcl con slot 0 SIN MLC (asignaciÃ³n sin bloque previo).
...
def test_slot_map_vacio(tmp_path: Path) -> None:
    """slot_map sin slot 0 â†’ ValueError (fail-fast en __init__)."""
...
def test_slot_0_sin_mlc_existente_crea_uno(tmp_path: Path) -> None:
    """El .s7dcl no tiene MLC para el slot 0 â†’ updater crea uno."""
    ...
    assert 0 in result.inserted  # slot 0 fue insertado (no habÃ­a MLC previo)
...
def test_slot_0_con_mlc_existente_lo_respeta(tmp_path: Path) -> None:
    """TIA ya tiene un MLC para slot 0 â†’ NO se regenera."""
    dcl, res = _write_pair(tmp_path, S7DCL_WITH_SLOT0_MLC, S7RES_WITH_OLD)
    updater = DispCommentUpdater(
        dcl, res, slot_map={0: "NO USAR", 1: "Bomba 1"}, db_array_name="ED"
    )
...
def test_slot_nuevo_inserta_asignacion_y_mlc(tmp_path: Path) -> None:
    """Slot sin asignaciÃ³n previa â†’ updater crea bloque + asignaciÃ³n."""
...
"""
    ...
    assert "END_DATA_BLOCK" in new_dcl  # la inserciÃ³n se hizo antes del END_DATA_BLOCK


def test_slot_existente_sin_mlc_lo_anade(tmp_path: Path) -> None:
    """AsignaciÃ³n presente sin S7_MLC â†’ updater aÃ±ade SOLO el bloque."""
    s7dcl = """DATA_BLOCK DB2000_ED
    VAR
        "ED" : Array[0..10] of _.UDT_ZC_DISP_ED;
    END_VAR
...
"""
    ...
    assert 1 in result.inserted
    new_dcl, _ = _read_pair(dcl, res)
    # El bloque se aÃ±adiÃ³ justo antes de ED[1] := ();
    # Y la asignaciÃ³n ED[1] := (); sigue presente.
...
def test_elimina_mlc_huerfano_del_s7res(tmp_path: Path) -> None:
    """Entrada en .s7res que ya no se referencia â†’ desaparece."""
    s7dcl = """DATA_BLOCK DB2000_ED
    VAR
        "ED" : Array[0..10] of _.UDT_ZC_DISP_ED;
    END_VAR
...
def test_preserva_mlcs_de_cabecera_y_array(tmp_path: Path) -> None:
    """REGRESIÃ“N: los MLCs de cabecera del bloque (BlockComment, BlockTitle,
...
"""
    ...
    dcl, res = _write_pair(tmp_path, s7dcl, s7res)
    # Slot_map: solo modificamos el slot 0 (que ya tenÃ­a MLC_old0).
    updater = DispCommentUpdater(
    ...
    for mlc_id in ("MLC_block_cmt", "MLC_block_title", "MLC_arr_cmt"):
        assert f"id: {mlc_id}" in new_res, (
            f"MLC de cabecera {mlc_id!r} fue borrado por _prune_s7res (regresiÃ³n)."
        )
    # El MLC del slot 0 tambiÃ©n.
    assert "id: MLC_old0" in new_res


def test_extract_all_mlcs_from_s7dcl_cubre_tres_formatos(tmp_path: Path) -> None:
    """El mÃ©todo ``_extract_all_mlcs_from_s7dcl`` captura los 3 formatos de
...
def test_idempotencia_doble_apply(tmp_path: Path) -> None:
    """Aplicar el mismo slot_map 2 veces produce archivos byte-idÃ©nticos."""
    dcl, res = _write_pair(tmp_path, S7DCL_WITH_SLOT0_MLC, S7RES_WITH_OLD)
...
def test_escape_comillas_dobles_en_texto(tmp_path: Path) -> None:
    """Texto con comillas dobles â†’ escapado en ambos archivos."""
    dcl, res = _write_pair(tmp_path, S7DCL_NO_SLOT0_MLC, S7RES_EMPTY)
    updater = DispCommentUpdater(
        dcl, res,
...
def test_texto_vacio_se_convierte_a_punto(tmp_path: Path) -> None:
    ...
    # Buscar la lÃ­nea de ese MLC concreto.
    ...
    for i, line in enumerate(lines):
        ...
        pytest.fail(f"No se encontrÃ³ la entrada de {result.inserted[1]} en .s7res")


def test_unicode_acentos_en(tmp_path: Path) -> None:
    """Caracteres no-ASCII (Ã±, tildes) se preservan en UTF-8."""
    dcl, res = _write_pair(tmp_path, S7DCL_NO_SLOT0_MLC, S7RES_EMPTY)
    updater = DispCommentUpdater(
        dcl, res, slot_map={0: "NO USAR", 1: "MÃ³dulo de mediciÃ³n"}, db_array_name="ED"
    )
    updater.update()
    updater.save()
    _, new_res = _read_pair(dcl, res)
    assert "MÃ³dulo de mediciÃ³n" in new_res


def test_was_modified_true_si_cambia_algo(tmp_path: Path) -> None:
    """was_modified devuelve True si el updater modificÃ³ el contenido."""
    dcl, res = _write_pair(tmp_path, S7DCL_NO_SLOT0_MLC, S7RES_EMPTY)
    updater = DispCommentUpdater(
        dcl, res, slot_map={0: "NO USAR", 1: "X"}, db_array_name="ED"
    )
...
def test_was_modified_false_si_idempotente(tmp_path: Path) -> None:
    """Re-aplicar el mismo slot_map â†’ was_modified == False la 2Âª vez."""
    dcl, res = _write_pair(tmp_path, S7DCL_WITH_SLOT0_MLC, S7RES_WITH_OLD)
...
def test_db_array_name_vacio_falla(tmp_path: Path) -> None:
    """db_array_name vacÃ­o â†’ ValueError."""
    dcl, res = _write_pair(tmp_path, S7DCL_NO_SLOT0_MLC, S7RES_EMPTY)
...
def test_no_modifica_otros_arrays(tmp_path: Path) -> None:
    ...
    updater.save()
    new_dcl, _ = _read_pair(dcl, res)
    # La secciÃ³n de Agrup debe seguir intacta (no la hemos tocado).
    assert "Agrup" in new_dcl
    # Agrup estÃ¡ declarado en la secciÃ³n VAR, no inicializado en el fixture.
    assert "Agrup : Array" in new_dcl
    # La secciÃ³n de inicializaciÃ³n del array ED sigue conteniendo sus
