...
def test_register_accepts_tia_client_not_dict():
    """register(tia_client) funciona; no requiere dict."""
    client = _fresh_client()
    register(client)
    assert len(client.registered_commands()) > 0


def test_register_registers_all_disp_commands_per_hw_type():
...
def test_register_registers_all_proc_commands_per_kind():
...
def test_register_registers_proc_param_combined_handler():
    client = _fresh_client()
    register(client)
    assert client.has_command("update_proc_comments_db_param")


def test_register_registers_online_and_offline_commit_handlers():
    client = _fresh_client()
    register(client)
...
def test_register_keeps_deprecated_commit_devices_sync_for_compat():
...
def test_register_total_count_is_consistent():
...
def test_registered_handlers_have_ob1_signature():
    ...
    _wrap_handler es invocable con 2 args y devuelve dict.
    """
    client = _fresh_client()
    register(client)

    # Tomamos uno cualquiera (ping no esta; usamos open_new_portal que SI esta
    # en core commands). Para area, usamos update_disp_comments_db_ed.
    cmd = "update_disp_comments_db_ed"
    assert client.has_command(cmd)

    # Llamamos con args vacios y un mock tia_client (los handlers internos
    # fallaran al intentar usar portal/ts reales, pero eso es OK: el wrapper
    # extrae wrapper/ts del mock y el handler cae en RuntimeError).
    mock_tia = MagicMock()
    mock_tia.wrapper = None  # fuerza RuntimeError en el handler interno
    mock_tia.ts = None

    # El wrapper devuelve lo que el handler devuelva. Si el handler lanza,
    # el wrapper deja propagar la excepcion (el dispatcher la captura).
    # Aqui testeamos directamente, sin pasar por dispatch.
    import inspect
    handler = client._handlers[cmd]
    # El wrapper acepta (args, tia_client).
