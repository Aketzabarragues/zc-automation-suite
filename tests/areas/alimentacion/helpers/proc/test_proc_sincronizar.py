...
def test_open_transaction_two_import_blocks_per_db(
    make_ctx: Any,
) -> None:
    """Con cambios en commits, 2 ``import_block`` separados (1 PARAM + 1 ALM).

    Sept-2026 fix (Commit 28, regression del 27): UN solo
    ``import_block`` combinado hacia que TIA V21 fallase
    silenciosamente con "Import failed because an object with
    the name X already exists in the plc" incluso con archivos
    validos (validado por el operario en VM, mismo ``modified
    _bloques/`` que importa OK con script standalone). Restaurado
    el patron legacy: 1 ``import_block`` por DB que tuvo cambios,
    cada uno con ``import_dir`` apuntando al subpath COMPLETO del
    DB (no la raiz ``modified_bloques/``), con ``target_folder=" "``.

    El test usa ``ArrayCommitResult`` con ``injected`` no vacio
    para forzar ``param_modified=True`` y ``alm_modified=True``,
    y captura todos los ``submit_and_wait`` para verificar la
    forma del dispatch.
    """
    import asyncio
    from unittest.mock import patch
    ...
    # Cada commit devuelve 1 injected (fuerza modified=True).
    def fake_commit(dcl_path, res_path, array_name, slot_map, **kwargs):
        assert kwargs.get("write_to_original") is True
        result = ArrayCommitResult(array_name=array_name)
        result.injected[1] = "MLC_test"
        return result
    ...
    # Filtramos los import_block (debe haber 2: PARAM + ALM).
    import_calls = [
        (cmd, args) for cmd, args in captured if cmd == "import_block"
    ]
    assert len(import_calls) == 2, (
        f"esperaba 2 import_block (PARAM + ALM), obtuvo {len(import_calls)}"
    )

    # Cada import tiene import_dir apuntando al subpath COMPLETO del DB
    # (no la raiz modified_bloques/) y target_folder="".
    import_dirs = [args["import_dir"] for _, args in import_calls]
    assert any("53010_Parametros" in d for d in import_dirs), (
        f"esperaba un import_dir con subpath PARAM, obtuvo: {import_dirs}"
    )
    assert any("55010_Alarmas" in d for d in import_dirs), (
        f"esperaba un import_dir con subpath ALM, obtuvo: {import_dirs}"
    )
    for _, args in import_calls:
        assert args["target_folder"] == ""
        # import_dir NO es la raiz modified_bloques/.
        assert not args["import_dir"].rstrip("\\/").endswith("modified_bloques"), (
            f"import_dir debe apuntar al subpath del DB, no a la raiz: "
            f"{args['import_dir']}"
        )
