...
def mock_tia_client() -> MagicMock:
    """Gateway mockeado: ambos metodos export son AsyncMock."""
    client = MagicMock()
...
def test_export_and_diff_handles_export_failure(
    ...
    """Si tia_client.export_block falla -> current=None + export_error poblado."""
    import asyncio
    from unittest.mock import patch
    import areas.alimentacion.data.data_ProcSlotMap as data_mod
    ...
    # tia_client.export_block falla con RuntimeError
    mock_tia = MagicMock()
    mock_tia.export_block = AsyncMock(
        side_effect=RuntimeError("TIA Portal no responde")
    )
    mock_tia.export_plc_tags_xml = AsyncMock(return_value=None)
