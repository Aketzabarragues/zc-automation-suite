...
Nota sobre "refrescar" el Excel (sept-2026, pedido operario):
los navegadores **NO exponen la ruta completa del archivo
original** (restriccion W3C de seguridad: el File API solo expone
basename, size y tipo). Por tanto, **no podemos re-leer el
archivo del operario desde disco sin su intervencion**. El
operario refresca pulsando "🔄 Volver a seleccionar" en la SPA
(boton que dispara el file picker; al re-seleccionar el mismo
archivo, el ``@change`` del input llama a ``handleExcel``,
mismo flujo que la primera carga). Este diseno es honesto y
