...
class DispEAParser:
    """Parser de la ``Tabla_Disp_EA`` (hoja ``DISP_EA``).

    Atributos de clase:
        * ``SHEET``: nombre literal de la hoja (``"DISP_EA"``).
        * ``TABLE``: nombre de la ``ListObject``
          (``"Tabla_Disp_EA"``).

    Si se inyecta un ``ConfigManager`` con un ``excel_target``
    para ``"ea"``, los nombres se sobreescriben en ``__init__``.
    """
    ...
    def __init__(self, config_manager: ConfigManager | None = None) -> None:
        if config_manager is not None:
            target = config_manager.get_excel_target_for("ea")
