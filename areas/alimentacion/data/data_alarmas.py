"""Data Block de DataAlarmaPLC.

Una **alarma** es un bit de un DB de alarmas (``DB{num_db}``) que el
HMI monitoriza para señalizar un evento. Las alarmas se agrupan en un
DB por proceso: ``DB{num_db}_{proceso_codigo}_ALM`` (uno por proceso,
contiene varios ``DataAlarmaPLC`` consecutivos en una
``ARRAY[0..N] OF BOOL`` o similar).

Más simple que ``DataParamRealPLC`` / ``DataParamIntPLC`` (7 campos, sin
``Visibilidad``/``Producto``/``Tipo``/``num_lista``/``txt_lista``). La
razón es histórica: el legacy TUI define ``Alarma`` con solo 6 columnas
en la ``ListObject`` (``UID``, ``Numero``, ``Proceso``, ``Num.DB``,
``Descripcion``, ``ComentarioDB``); ``Visibilidad`` no existe en esta
tabla.

Defensa contra schema drift: si el Excel del corporativo incluye en el
futuro una columna ``Visibilidad`` en ``Tabla_Alarmas``, el parser la
**ignora silenciosamente**. El DTO ``DataAlarmaPLC`` NO tiene atributo
``visibilidad`` por diseño.
"""
