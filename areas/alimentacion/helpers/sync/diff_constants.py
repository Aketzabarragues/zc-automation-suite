"""Motor puro de diffs para PlcUserConstant del area de alimentacion.

Recibe el estado actual del PLC (exportado a XML o leido por COM) y el
estado deseado (del Excel), y devuelve la lista de operaciones necesarias
para sincronizar el PLC bajo una transaccion atomica.

Este modulo es **especifico del area de alimentacion** (las PlcUserConstant
N_MAX ``N_MAX_DISP_*`` y las PlcUserConstant de dispositivos ``V_*``
viven en este dominio). Si en el futuro otro departamento necesita
diffs equivalentes, se movera a una ubicacion compartida.
...
requieren logicas de diff diferentes:

1. **N_MAX (dimensiones)**
   - El nombre es la KEY estable (``N_MAX_DISP_ED``, ``N_MAX_DISP_EA``...).
   - Lo que cambia es el VALOR (ej. ``25`` -> ``30``).
   - Operacion: ``update_user_constant_value``.

2. **Dispositivos**
   - El VALOR es el UID estable del dispositivo (entero unico).
   - Lo que cambia es el NOMBRE (etiqueta humana).
   - Operacion: ``update_user_constant_name``.

Esta separacion es FUNDAMENTAL porque mezclar ambos diffs lleva a errores
...
def calculate_nmax_diff(
    ...
    Returns:
        ...
        con shape compatible con ``execute_transactional_batch``.

    Notas:
        - **Create**: NO se emite. Si una constante N_MAX no existe en
          TIA pero si en el Excel, se ignora (las N_MAX son un conjunto
          cerrado y conocido).
    ...
    for constant_name, desired_value in desired_state.items():
        ...
        if current_value is None:
            # La constante no existe en TIA -> no la creamos via COM.
            # (Coherente con la politica 'N_MAX es conjunto cerrado'.)
            continue
        if current_value == int(desired_value):
            continue  # ya esta en el valor deseado
        operations.append(
            {
...
def calculate_device_rename_diff(
    ...
    Notas:
        ...
        - Si un valor existe en desired pero no en current -> se ignora
          (caso de dispositivo nuevo; requiere inyeccion XML offline).
        - Si un valor existe en current pero no en desired -> se ignora
          (caso de dispositivo eliminado; requiere eliminacion COM
          explicita o de tabla entera).
    """
    operations: list[dict[str, Any]] = []
...
__all__ = ["calculate_nmax_diff", "calculate_device_rename_diff"]
