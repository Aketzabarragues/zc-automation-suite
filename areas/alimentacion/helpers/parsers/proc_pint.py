"""Parser de parÃ¡metros enteros del Excel corporativo.
...
Diferencias con el legacy TUI (``_legacy_reference/ZC_ALM_TOOLS``):
    ...
    * Mismas claves literales del Excel que el legacy
      ...
      software/pint.py`` lÃ­neas 27-38): ``UID``, ``Numero``,
      ``Proceso``, ``Codigo``, ``Num.DB``, ``Producto``, ``Tipo``,
      ``Descripcion``, ``ComentarioDB``, ``Visibilidad``,
      ``Num.Lista``, ``Txt.Lista``.

Punto crÃ­tico de Fase 3 (heredado de R4 del plan, ya tratado en
Fase 2): el campo ``num_lista`` **no** se mapea con ``_safe_int``
(que destruirÃ­a los marcadores semÃ¡nticos del operario cayendo a
...
aunque el shape (12 campos) es idÃ©ntico, los dos DTOs son
**nominalmente distintos** en Python. El parser usa su propio DTO
(``ParamIntPLC``) â€” no se reutiliza ``ParamRealPLC``. Si en el
futuro se quiere aÃ±adir ``rango_min``/``rango_max`` solo a
...
class PIntParser:
    ...
    def extraer(self, wb: Workbook) -> list[ParamIntPLC]:
        """Extrae todos los parÃ¡metros enteros del workbook.
        ...
        PolÃ­tica de descarte (consistente con ``ProcesosParser`` y
