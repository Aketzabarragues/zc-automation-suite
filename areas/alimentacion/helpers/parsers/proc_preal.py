"""Parser de parÃƒÂ¡metros reales del Excel corporativo.
...
de ``DataParamRealPLC`` (DTO inmutable definido en
...
Diferencias con el legacy TUI (``_legacy_reference/ZC_ALM_TOOLS``):
    ...
    * Mismas claves literales del Excel que el legacy
      ...
      software/preal.py`` lÃƒÂ­neas 25-38): ``UID``, ``Numero``,
      ``Proceso``, ``Codigo``, ``Num.DB``, ``Producto``, ``Tipo``,
      ``Descripcion``, ``ComentarioDB``, ``Visibilidad``,
      ``Num.Lista``, ``Txt.Lista``.

Punto crÃƒÂ­tico de Fase 2 (R4 del plan): el campo ``num_lista``
**no** se mapea con ``_safe_int`` (que destruirÃƒÂ­a los marcadores
semÃƒÂ¡nticos del operario cayendo a ``0``). Se usa
...
class PRealParser:
    ...
    Mapea cada fila de la ``ListObject`` a un ``DataParamRealPLC``. Las
    ...
    def extraer(self, wb: Workbook) -> list[DataParamRealPLC]:
        """Extrae todos los parÃƒÂ¡metros reales del workbook.

        Args:
            wb: workbook de openpyxl ya abierto (no se cierra aquÃƒÂ­).

        Returns:
            Lista de ``DataParamRealPLC``. Si la hoja o la tabla no
            existen (R1 del plan), devuelve ``[]``. Las filas que
            fallen al construir el DTO se descartan con WARNING.

        PolÃƒÂ­tica de descarte (consistente con ``ProcesosParser``,
        legacy dropna por UID):
            * Filas con ``UID`` vacÃƒÂ­o (``None`` / ``""`` / whitespace
              / ``"nan"`` / ``"None"`` / ``"null"``) se descartan
              silenciosamente. Esto evita parÃƒÂ¡metros fantasma sin
              UID en el cache. Es el equivalente del
              ``pandas.dropna(subset=["UID"])`` del legacy TUI.
            * Filas con ``UID`` no vacÃƒÂ­o se conservan aunque el
              resto de campos estÃƒÂ© vacÃƒÂ­o: el DTO tiene defaults
              tolerantes (``str = ""``, ``int = 0``,
              ``num_lista = 0``).
        """
        rows = extract_list_object_rows(wb, self.SHEET, self.TABLE)
        result: list[DataParamRealPLC] = []
        for row in rows:
