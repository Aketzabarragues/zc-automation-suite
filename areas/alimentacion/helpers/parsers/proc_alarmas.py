...
Diferencias con el legacy TUI (``_legacy_reference/ZC_ALM_TOOLS``):
    ...
    * Mismas claves literales del Excel que el legacy
      ...
      software/alarmas.py`` lÃ­neas 23-30): ``UID``, ``Numero``,
...
Esta es la **implementaciÃ³n de referencia** que los 6 mini parsers
de dispositivos de Fase 5 (``disp_ed.py``, ``disp_ea.py``,
``disp_sa.py``, ``disp_v.py``, ``disp_m.py``, ``disp_m_vf.py``)
imitarÃ¡n en estructura: cada uno es una clase con
``SHEET``/``TABLE`` como constantes y un Ãºnico mÃ©todo
``extraer(self, wb: Workbook) -> list[DTO]`` que delega en
``extract_list_object_rows`` y descarta filas invÃ¡lidas con
``logger.warning``. Los parsers de Fase 5 serÃ¡n casi 1:1 con
``AlarmasParser`` cambiando solo el DTO destino y las claves de
columna (R5 del plan, R-F4.1 tambiÃ©n).

R-F4.1 (defensa contra schema drift): el mÃ©todo ``extraer`` solo
...
corporativo decide aÃ±adirla en el futuro), esas columnas se
ignoran silenciosamente: el ``dict`` que devuelve
``extract_list_object_rows`` las contendrÃ¡ como claves, pero el
constructor de ``AlarmaPLC`` no las acepta (``frozen=True`` con
lista cerrada de campos) y el ``try/except`` que rodea la
construcciÃ³n las descarta con WARNING. El test
...
class AlarmasParser:
    ...
    ImplementaciÃ³n de referencia: este parser es la plantilla
    que los 6 mini parsers de dispositivos (Fase 5.3 del plan)
    imitarÃ¡n. La Ãºnica diferencia entre ``AlarmasParser`` y un
    futuro ``DispEDParser`` serÃ¡:
        ...
        * DTO destino (en este caso ``AlarmaPLC``, 6 campos; en
          Fase 5 serÃ¡ ``DispED``, 11+ campos, etc.).
        * Claves de columna leÃ­das (las 6 del legacy alarmas, o
    ...
    def extraer(self, wb: Workbook) -> list[AlarmaPLC]:
        ...
        Descarta las filas con UID vacÃ­o y las que fallen al
