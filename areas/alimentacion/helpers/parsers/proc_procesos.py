...
del workbook del departamento de alimentaciÃ³n y la mapea a una lista de
...
Diferencias con el legacy TUI (``_legacy_reference/ZC_ALM_TOOLS``):
    * Recibe el workbook **ya abierto** (``wb: Workbook``). NO abre
      ...
      por parser de software) y soporta el patrÃ³n de Fase 5 donde el
    ...
    * Defensivo: cada fila se envuelve en ``try/except`` y las filas
      invÃ¡lidas se descartan con ``logger.warning`` (no rompen la
...
class ProcesosParser:
    ...
    claves de columna son **literales** (con mayÃºsculas y, en su
    caso, guion bajo) â€” exactamente como aparecen en el Excel del
    ...
    def extraer(self, wb: Workbook) -> list[ProcesoPLC]:
        ...
        PolÃ­tica de descarte (legacy, plan Â§5.7 test 5):
            ...
            * Filas con ``UID=0`` legÃ­timo (entero o ``"0"``) se
              conservan: ``uid=0`` es un valor vÃ¡lido en el Excel.
        """
        rows = extract_list_object_rows(wb, self.SHEET, self.TABLE)
        result: list[ProcesoPLC] = []
        for row in rows:
            # PolÃ­tica legacy: dropna por UID. ``_safe_str`` ya mapea
            # ``None`` / ``""`` / ``"nan"`` / ``"None"`` / ``"null"`` a
            # ``""``. Un UID ``"0"`` o ``0`` pasa el filtro (es vÃ¡lido).
