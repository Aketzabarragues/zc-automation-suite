...
_logger: logging.Logger = logging.getLogger(f"{__name__}.DispMLCRegistry")
...
class DispMLCRegistry:
    ...
    def reserve(self, used_ids: Iterable[str]) -> None:
        ...
        if added:
            _logger.debug(
                f"DispMLCRegistry.reserve: {added} IDs nuevos añadidos "
    ...
    def next_mlc_id(self) -> str:
        ...
        for _ in range(_MAX_ATTEMPTS):
            ...
            if candidate not in self._used:
                ...
                return candidate
        raise RuntimeError(
            f"DispMLCRegistry: no se pudo generar un MLC único tras "
